export default function Searchbar(prop){
    return(
    <form onSubmit = {prop.handleSubmit} className={`${prop.dark ? "bg-navy-blue" : "bg-white"} flex justify-between rounded-xl mt-8 text-xs 480:text-base shadow-2xl`}>
        ...
        <input 
            className={`${prop.dark ? "bg-navy-blue" : "bg-white"} ${prop.dark ? "text-white" : "text-black"} ${prop.dark ? "placeholder-white" : "placeholder-grayish-blue"} w-full outline-none  text-ellipsis`} 
            type="text" 
            name="vin" 
            id="vin" 
